...
$trialExpiry = get_site_option('btbbabfte');
...
<div id="fl-bt_bb_ab_test-form" class="fl-settings-form">
  ...
  <form id="bt-bb-ab-form" action="<?php echo esc_html(BT_BB_AB_Admin::get_current_settings_url()); ?>" method="post">
    ...
    <?php if(!is_plugin_active_for_network(BT_AB_PLUGIN_FOLDER.'/bt-bb-ab.php') || is_network_admin()){ // dont show if network activated ?>
      ...
      <?php 
      if($trialExpiry > gmdate('Ymd') && ($license_status == 'valid')){
        $trialExpiryNice = gmdate('jS M Y', strtotime($trialExpiry));
        echo "<div class='free-trial-notice'>
        <h3>🎉 Welcome to a 7-Day Free Trial of Pro!</h3>
        <p>Discover the full power of AB Split Test with<strong> unlimited tests, unlimited variations, subgoals, AI, custom conversion values, analytics integrations, and white-label reporting.</strong></p>        <p>👉 Your free trial ends on: <strong>" . esc_html($trialExpiryNice) . "</strong></p><p><strong>Don't lose access to these premium features</strong>—upgrade now and keep all your data and advanced capabilities!</p>
        <a href='https://absplittest.com/pricing?utm_source=ugft' target='blank'>Upgrade Here.</a>
        <p><strong>💡 After your trial ends:</strong> You'll be switched to the free version with limited features: 1 active test, 1 variation, and no AI or subgoals.</p>
      </div>";
        $user_level = 'agency'; //to show the settings 
      }
      ?>
  ...
  "cline.mcpServers": {
    "<?php echo esc_html($mcpServerName); ?>": {
      ...
      "env": {
        ...
        <div class="floating-save-button-row"><input type="submit" class="button-primary" name="bt_save" value="<?php _e('Save Settings', 'bt-bb-ab'); ?>" /></div>
