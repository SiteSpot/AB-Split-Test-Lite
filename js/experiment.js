...
jQuery(document).ready(function() {
  ...
  jQuery('#configuration_settings > div > h3').on('click', function() {
  ...
  if(jQuery('body').hasClass('post-php'))
    jQuery( '<button class="button button-primary button-large" id="starttest">Update Test</button>' ).insertAfter( "#publish" );
  else
    jQuery( '<button class="button button-primary button-large" id="starttest">Start Test</button>' ).insertAfter( "#publish" );

  jQuery('#starttest').click(function(e){
    e.preventDefault();

    if(!jQuery('#full_page').is(':checked') &&  !jQuery('#ab_test').is(':checked')&&  !jQuery('#css_test').is(':checked') && !jQuery('#magic').is(':checked'))
    {
  ...
  jQuery('.submitdelete.deletion').text('Cancel test');
  ...
  if(jQuery('.results_variation').length >1)
  {
    ...
    jQuery('[href="#results"]').addClass('tab-active');
    jQuery("#configuration_settings").hide();
  ...
  jQuery('[href="#config"]').on('click',function(e){
    ...
    jQuery(".show_experiment_results").hide();
  });
  ...
  jQuery('.goal-select').change(function(e){
   ...
   //show goal then append span inside thje tag
    ...
    // Initial update of the description when the page loads
  ...
  jQuery('#css_test_variations').on('change',function(e){
    ...
    for (var i = 1; i <= parseInt(jQuery('#css_test_variations').val()); i++) {
        jQuery(".css-test-helper-zone").append('<pre><span style="color:grey;"> body.</span>test-css-'+testId+'-'+i+'</pre>');
    }
  ...
  jQuery("#bt_experiments_conversion_page").change(function(){
    ...
    if( selectval == 'woo-order-received' || selectval == 'surecart-order-paid' )
  ...
  jQuery('.results_variation .title').on('click', function(e) {
    if (e.detail !== 3) return; // triple click
    // confirm if delete?
    if(confirm('Delete Variation data for ' + jQuery(this).text() + '?')){

      console.log('Delete ' + jQuery(this).text() + ' ID:' + window.abstpid );
...
// Safety check for control variation
if(!abtestChartData.observations || !abtestChartData.control_variation || !abtestChartData.observations[abtestChartData.control_variation]) {
  console.log('Missing control variation data');
  return;
}
var controlVariationRate = abtestChartData['observations'][abtestChartData.control_variation]['rate'];
for (let observationKey in abtestChartData.observations) {
    ...
    var lift_raw = Math.round(((observation.rate - controlVariationRate) / controlVariationRate) * 100 * 10) / 10;
...
chance_of_winning_label = "Chance of<br/>Winning";
...
var table = new Tabulator("#abst-results-table", {
  ...
  columns:[
      {title:" ", field:"link", visible:true, headerSort:false, width:50, formatter:"html"},
...
setTimeout(function() {
  ...
  jQuery('#abst-results-table .tabulator-row').each(function() {
    ...
    if(id == abtestChartData.test_winner){
...
jQuery('#abst-goal-select').on('change', function() {
...
function refreshConversionPage(){
  ...
  conv_page = jQuery("#bt_experiments_conversion_page").val();
  if(conv_page == "false" || conv_page == "0" || conv_page == "url" || conv_page == "embed" ||  conv_page == ""  ||  conv_page == "selector"  ||  conv_page == "javascript" || conv_page == "time"  || conv_page == "woo-order-pay"  || conv_page == "woo-order-received" || conv_page == "text" || conv_page == "surecart-order-paid"  || conv_page == "link" || conv_page == "scroll")
  {
    jQuery("#bt_experiments_conversion_page_preview").hide();
  }
  else
  {
    jQuery("#bt_experiments_conversion_page_preview").show().attr("href",bt_homeurl + "/?page_id="+jQuery("#bt_experiments_conversion_page").val());
  }
}
