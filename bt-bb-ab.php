...
if (!defined('BT_AB_TEST_WL_NAME')) define('BT_AB_TEST_WL_NAME', apply_filters('ab_wl_name', 'AB Split Test Lite'));
...
// AI / OpenAI integration removed in lite version
// define('BT_AB_AI_MODEL','gpt-4.1');

// Premium module loaders removed in lite version
// require 'conversionpixel.php';
// require 'includes/plugin-update-checker/update-checker.php';
// require 'modules/forms/form-conversions.php';

// $bt_bb_ab_update_checker = new BT_BB_AB_Update_checker;
...
if(! class_exists ( 'Bt_Ab_Tests'))
{
  class Bt_Ab_Tests{
    ...
    function __construct(){
      ...
      add_action('template_redirect', [$this, 'handle_abtest_query_param'], 1);
    ...
    function add_split_test_params_to_bakery() {
        ...
        vc_add_param('vc_row', array(
            'type' => 'autocomplete',
            'heading' => __('Choose a Test or  <a class="new-on-page-test-button" style="display:inline-block;margin-bottom:10px;" href="javascript:void(0);"> Create a new test</a>', 'bt-bb-ab'),
            'param_name' => 'test_id',
            'description' => __('Choose an existing split test', 'bt-bb-ab'),
            'group' => __('Split Test Settings', 'bt-bb-ab'),
        ));
        vc_add_param('vc_row', array(
            'type' => 'textfield',
            'heading' => __('Test Variation', 'bt-bb-ab'),
            'param_name' => 'test_variation',
            'description' => __('Enter the test variation name.', 'bt-bb-ab'),
            'group' => __('Split Test Settings', 'bt-bb-ab'),
        ));
    ...
    function conversion_targeting(){
      ...
      echo "<script ".ABST_CACHE_EXCLUDES." id='abst_conv_details'>
    ...
    function build_status_lifecycle_panel($post) {
      ...
      if ($status === 'idea') {
        $panel['rows'] = [
          ...
          ['icon' => 'calendar-alt', 'label' => 'Created', 'value' => $this->format_status_lifecycle_date($created_at)],
        ];
        $panel['primary_action'] = ['label' => 'Convert to Draft Test', 'action' => 'start-as-draft', 'class' => 'button button-primary button-large'];
        if ($trash_link) {
          $panel['footer_links'][] = ['label' => 'Delete idea', 'href' => $trash_link, 'class' => 'submitdelete deletion'];
        }
...
// Check if Thompson Sampling is enabled globally
  ...
  ';
    ...
    function show_login_targeting_options( $post )
    {
      ...
      echo '<div class="ab-targeting-roles"><h4>'. esc_html__('User roles', 'bt-bb-ab') .'</h4>';      
    ...
    function ab_split_test_report($atts){
      ...
      if(!apply_filters( 'bt_ab_shortcode', true ))
        return BT_AB_TEST_WL_NAME.": Shortcode Disabled.";
    
      //agency only soz
      $agency = btab_user_level() == 'agency';
      if(!$agency)
        return BT_AB_TEST_WL_NAME.": Upgrade to use reports.";
...
function cmp_by_ConversionRate($a, $b) {
  ...
  return $rateB - $rateA;
    ...
    function bt_experiments_inner_custom_box( $post ) {
      ...
      $select .="<option value='embed'>Conversion Pixel (same domain only)</option>";
      ...
      if (defined('WC_VERSION')){
        ...
        foreach($woo_pages as $name => $page)
        {
          ...
          {
            if(empty($woo_opts))      
              $woo_opts .= "</optgroup><optgroup label='WooCommerce'>";
            $woo_opts .= "<option value=\"$page\">$name</option>";
          }
        }
        // add in payment page and order recieved page
        $woo_opts .= "<option value=\"woo-order-pay\">Checkout Payment Page</option>";
        $woo_opts .= "<option value=\"woo-order-received\">Checkout Order Received (Thank You Page)</option>";
        $woo_opts .= "</optgroup>";
        // end woo
      }
      ...
      if ( class_exists( 'SURECART' ) ) {
          ...
          $woo_opts .= "</optgroup><optgroup label='SureCart'>";
          $woo_opts .= "<option value=\"surecart-order-paid\">Order Paid</option>";
          $woo_opts .= "</optgroup>";
      }

      //fluentcart
      if (function_exists('fluent_cart_scheduler_register')) {
        $woo_opts .= "</optgroup><optgroup label='FluentCart'>";
        $woo_opts .= "<option value=\"fluentcart-order-paid\">Order Paid</option>";
        $woo_opts .= "</optgroup>";
      }

      if(class_exists( 'Easy_Digital_Downloads' )){
          $woo_opts .= "</optgroup><optgroup label='Easy Digital Downloads'>";
          $woo_opts .= "<option value=\"edd-purchase\">Purchase</option>";
          $woo_opts .= "</optgroup>";
      }


      if ( class_exists( 'WPPIZZA' ) ) {
              $woo_opts .= "</optgroup><optgroup label='WP Pizza'>";
          $woo_opts .= "<option value=\"wp-pizza-is-checkout\">Checkout Page</option>";
          $woo_opts .= "<option value=\"wp-pizza-is-order-history\">Order History Page</option>";
          $woo_opts .= "</optgroup>";
      }
...
//     function log_experiment_activity($bt_eid = null, $bt_variation = null, $bt_type = null, $from_api = false, $bt_location = false, $abConversionValue = false,$uuid = false,$size = false, $advancedId = false){
    ...
    $labels = array(
      'name'                  => __( BT_AB_TEST_WL_NAME, 'Post Type General Name', 'bt-bb-ab' ),
      'singular_name'         => __( BT_AB_TEST_WL_NAME, 'Post Type Singular Name', 'bt-bb-ab' ),
      'menu_name'             => __( BT_AB_TEST_WL_NAME, 'bt-bb-ab' ),
      'name_admin_bar'        => __( BT_AB_TEST_WL_NAME, 'bt-bb-ab' ),
      'archives'              => __( BT_AB_TEST_WL_NAME.' Archives', 'bt-bb-ab' ),
      'attributes'            => __( BT_AB_TEST_WL_NAME.' Attributes', 'bt-bb-ab' ),
      'parent_item_colon'     => __( 'Parent '.BT_AB_TEST_WL_NAME, 'bt-bb-ab' ),
      'all_items'             => __( 'All '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'add_new_item'          => __( 'New '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'add_new'               => __( 'New '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'new_item'              => __( 'New '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'edit_item'             => __( 'Edit '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'update_item'           => __( 'Update ' .BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'view_item'             => __( 'View', 'bt-bb-ab' ),
      'view_items'            => __( 'View ' .BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'search_items'          => __( 'Search ' .BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      ...
      'items_list'            => __( BT_AB_TEST_WL_ABTEST.' list', 'bt-bb-ab' ),
      'items_list_navigation' => __( BT_AB_TEST_WL_ABTEST.' list navigation', 'bt-bb-ab' ),
      'filter_items_list'     => __( 'Filter '.BT_AB_TEST_WL_ABTEST.' list', 'bt-bb-ab' ),
    );
    $args = array(
      'label'                 => __( BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'description'           => __( 'Page Builder '.BT_AB_TEST_WL_ABTEST, 'bt-bb-ab' ),
      'labels'                => $labels,
...
.bt-show-variation {
      ...
      function test_post_states( $states, $post ) {
          ...
          if ( 'bt_experiments' == get_post_type( $post->ID ) ) {
            ...
            if($post->post_status == 'idea')
              ...
              $newstate = '<span>ðŸ”´ Not Started </span>';
            else if($post->post_status == 'publish')         
              $newstate = '<span>ðŸŸ¢ Running</span>';
            else if($post->post_status == 'complete')         
              $newstate = '<span>âœ”ï¸ Complete</span>';
            else if($post->post_status == 'pending')         
              $newstate = '<span>â¸ï¸ Paused</span>';
    ...
    function render_admin_scripts_styles(){
      ...
      $abst_admin_pages = ['abst-heatmaps', 'abst-agency-hub', 'bt_bb_ab_insights', 'abst-session-replay'];
 ...
 */
    ...
    function register_abilities() {
      ...
      if (class_exists('WooCommerce'))                                          { $active_types[] = 'woo-order-received'; $active_types[] = 'woo-order-pay'; }
      if (class_exists('Easy_Digital_Downloads'))                               { $active_types[] = 'edd-purchase'; }
      if (class_exists('SureCart'))                                             { $active_types[] = 'surecart-order-paid'; }
      if (function_exists('fluentcart'))                                        { $active_types[] = 'fluentcart-order-paid'; }
      if (class_exists('WPPIZZA'))                                              { $active_types[] = 'wp-pizza-is-checkout'; $active_types[] = 'wp-pizza-is-order-history'; }
    ...
    function rest_list_tests($request) {
      ...
      $args = [
        'post_type' => 'bt_experiments',
        'posts_per_page' => -1,
        'post_status' => ['idea', 'publish', 'draft', 'pending', 'complete'],
        'orderby' => 'date',
        'order' => 'DESC'
      ];
    ...
    /**
     * Handle ?ssr=1 parameter on any page
     * Server-side rendering mode - assigns variation and serves content without JS redirect
     */
    ...
    /**
     * Swap page content when abtest_query_variation is set
     */
    ...
    /**
     * Assign a variation for the ?t= parameter
     * Respects traffic percentage, user roles, and Thompson sampling
     */
    ...
    /**
     * Select variation using Thompson sampling weights — disabled in lite version
     */
    ...
    /**
     * Perform weighted random selection — disabled in lite version
     */
...
function abst_start_free_trial(){

  if(get_site_option('btbbabfte'))
    return; // already done
  
  $expiry = date('Ymd', strtotime('+7 days'));
  //set wordpress site option
  update_site_option('btbbabfte', $expiry);
  
  // Lite: heatmap trial removed - heatmaps are free with limits
}

function abst_extend_trial(){
  $current_expiry = get_site_option('btbbabfte');
  if(!$current_expiry) return false; // no trial to extend
  
  // Only allow extension once
  if(get_option('abst_trial_extended', false)) return false;
  
  // Extend by 7 days from current expiry
  $new_expiry = date('Ymd', strtotime($current_expiry . '+7 days'));
  update_site_option('btbbabfte', $new_expiry);
  
  // Mark as extended
  update_option('abst_trial_extended', true);
  
  return true;
}

function abst_handle_trial_extension(){
  if(!isset($_GET['extendtrial']) || !current_user_can('manage_options')) {
    return;
  }
  
  $extended = abst_extend_trial();
  
  if($extended) {
    add_action('admin_notices', function() {
      echo '<div class="notice notice-success is-dismissible">';
      echo '<p><strong>Trial Extended!</strong> Your trial has been extended by 7 days.</p>';
      echo '</div>';
    });
  } else {
    add_action('admin_notices', function() {
      echo '<div class="notice notice-warning">';
      echo '<p><strong>Trial Extension Failed:</strong> Trial can only be extended once, or no active trial found.</p>';
      echo '<p>If you need more time, please subscribe. We have a 14 day no-questions money back guarantee.</p>';
      echo '</div>';
    });
  }
}
...
function bt_bb_ab_filter_all_plugins( $get_plugins ) {
  ...
  if( !isset($wl->bb) ) { $wl->bb = ''; }
  if( !isset($wl->bt) ) { $wl->bt = ''; }
...
function ab_split_test_admin_bar_menu( $wp_admin_bar ) { 
    ...
    $args = array(
        'id'    => 'ab-test',
        'title' => '<div class="ab-test-tube"></div>'.esc_html(BT_AB_TEST_WL_NAME),
    );
...
function abst_add_logs_page() {
  ...
  if(ab_get_admin_setting('abst_agency_mode_enabled') == '1') {
    add_submenu_page(
      'edit.php?post_type=bt_experiments',
      'Agency Hub',
      'Agency Hub',
...
function abst_heatmaps_page_content() {
  ...
  // Left column
  echo '<div>';
  echo '<div class="abst-filter-group" style="margin-bottom: 15px;"><label>Page</label><div class="abst-filter-row">';
  echo '<span>' . esc_html($heatmap_page_title) . '</span> ';
  ...
  echo '<div class="abst-filter-group"><label>Days</label><div class="abst-filter-row">';
  echo '<a href="https://absplittest.com/pricing?ref=upgradefeaturelink" target="_blank" class="button button-secondary">Upgrade to extend data retention</a> 3 days';
  echo '</div></div>';
  echo '</div>';
  ...
  echo '</div>'; // Close abst-heatmaps-filters
    ...
    if (!$has_scroll_data && !$has_click_data && !$has_rage_data && !$has_dead_data) {
      ...
      $context_eid = !empty($context_parts['eid']) ? ' <span class="abst-context-sep">â€º</span> Test: ' . $context_parts['eid'] : '';
      $context_variation = !empty($context_parts['variation']) ? ' <span class="abst-context-sep">â€º</span> Variation: ' . $context_parts['variation'] : '';
      ...
      echo '<div class="abst-heatmap-context"><div><span class="abst-context-icon">ðŸ“Š</span><strong>'. $modeNice . ':</strong> ' . esc_html($context_parts['page']) . ' <span class="abst-context-sep">â€º</span> Size: '  . esc_html($context_parts['size']) . $context_eid . $context_variation . '</div>';
      echo '<p class="abst-heatmap-description">' . $description . '</p></div>';
      
      // Add legend for scroll mode
      if ($selected_mode === 'scroll') {
        echo '<div class="abst-scroll-legend" style="position: sticky; top: 40px; z-index: 1000; width: '.$screenWidth.'px; max-width:90%; margin: 20px auto 10px; padding: 20px 10px; background: #f5f5f5; border-radius: 8px; display: flex; align-items: center; gap: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">';
      ...
      echo '<div class="abst-heatmap-wrapper" style="position: relative; max-width: '.$screenWidth.'px; width:calc(100% - 20px); margin: 40px auto; border: 10px solid #d9d9d9; box-shadow: 0 1px 10px -4px black;">';
...
function abst_agency_hub_page_content() {
  // Agency Hub is PRO only — disabled in lite version
  echo '<div class="wrap"><h1>Agency Hub</h1><p>This feature is only available in the Pro version. <a href="https://absplittest.com/pricing?utm_source=ugagencyhub" target="_blank">Upgrade to unlock</a>.</p></div>';
  return;
}
...
function abst_logs_page_content() {
  ...
  echo '<div class="abst-page-header">';
  echo '<h1>'.BT_AB_TEST_WL_ABTEST.' Debug Logs</h1>';
  echo '</div>';
...
function sendReportEmail($emails){
  ...
  $email_subject = BT_AB_TEST_WL_NAME . ' Weekly Report';
  ...
  $email_message .= '<h1 style="font-size: 24px;">Your ' . BT_AB_TEST_WL_NAME . ' Weekly Report</h1>';
...
/**
 * Get or set the experiment variation for server-side rendering
 *
 * This function checks for existing cookies/sessions first, then applies targeting rules
 * to determine which variation a user should see for server-side changes.
 *
 * @param int $test_id The experiment ID
 * @return int|string|false Returns variation index for magic/code tests, page ID for full page tests, false for on-page tests
 */
...
/**
 * Parse variation string into appropriate return value based on test type
 *
 * @param string $variation The variation identifier
 * @param string $test_type The test type (full_page, magic, code)
 * @return int|false
 */
...
/**
 * Check if user should see the test based on targeting rules
 *
 * @param int $test_id
 * @param array $targeting_options
 * @return bool
 */
...
/**
 * Select a variation for the user based on test configuration
 *
 * @param int $test_id
...
/**
 * Select variation based on Thompson sampling weights
 *
 * @param array $variation_meta
 * @return string
 */
